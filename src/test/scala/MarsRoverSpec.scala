import MarsRover.Command.{Forwards, RotateClockwise}
...
//  }
  ...
  it should "find a best path and make rover move" in {
    val path = Pathfinder.shortestPath(3, GridSquare(1,1), GridSquare(2,2))
    val mountains = List(GridSquare(1,3), GridSquare(1,2), GridSquare(2,1))
    val rover = Rover(GridSquare(1,1), Up, List.empty, (3,3), GridSquare(2,2), mountains)
  ...
  it should "work out a zigzagging path" in {
    val end = GridSquare(3,3)

    Pathfinder.zigZagPath(xFirst = true, end).distinct shouldBe  List((2,1), (2,2), (3,2), (3,3))
    Pathfinder.zigZagPath(xFirst = false, end).distinct shouldBe List((1,2), (2,2), (2,3), (3,3))
  }

  it should "work out a reverse zigzagging path" in {
    val end = GridSquare(3,3)

    Pathfinder.zigZagReverse(xFirst = true, end, 4).distinct shouldBe  List((4,1), (4,4), (3,4), (3,3))
    Pathfinder.zigZagReverse(xFirst = false, end, 4).distinct shouldBe List((1,4), (4,4), (4,3), (3,3))
  }
